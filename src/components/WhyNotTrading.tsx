...
/** The "why isn't it trading" checklist. Both engines compute every gate's
 * block reason on every cycle — this surfaces them, ordered so the first red
 * row IS the answer. Polls while mounted; cheap (one RPC per 5s). */
  ...
  useEffect(() => {
    ...
    const pull = async () => {
      try {
        ...
        if (alive) setSt(s);
      } catch { /* backend restarting — keep the last snapshot */ }
    };
    void pull();
