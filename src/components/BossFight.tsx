...
/** Procedural pixel-monster, escalating menace by tier. */
...
function drawBluey(ctx: CanvasRenderingContext2D, sprite: HTMLImageElement, x: number, y: number, size: number, frame: number): void {
  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(sprite, (frame % 8) * 50, 0, 50, 50, x, y, size, size);
}
...
/** Bluey (left) vs the current boss (right) + an HP bar, drawn to fill (w,h). */
...
const WW = 420, WH = 92;   // widget buffer (2× of 210×46)
...
export function BossWidget() {
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      drawScene(ctx, WW, WH, sprite, frame, gRef.current, t, true);
...
const AW = 880, AH = 380;  // arena scene buffer
